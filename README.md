...
We check the deadlock of the system here using the command deadlocks (N.B. we made an error in the submitted paper about statistics which were not for the deadlocks, they will be corrected)

For the deadlock property, and because of the problem of state explosion in MWB verification tool, we have simplified the pi-calculus code generated by eliminating the match expressions to allow thread processes to not synchonize on matching tests. In addition, we have alleviated the schedular process declaration to have a sequential behavior. The aim is to helop reducing the generated state space which, consequetly, enables checking the deadlock property. Otherwise the tool continues to execute without returning any results and sometimes finishes the execution surprisingly (i.e. it can't handle the generation of the complete state space). The slightely changed version is given as follows:
...
**Liveness, Schedulability, Dispatch Exclusion** (c.f. Figure below)

These are the properties formulas elaborated in the Mads logic (adapted from mu-calculus). It is to note that **μ** (mu) in the standard modal μ-calculus corresponds to **min** in Mads Dam's notation — both denote the least fixed point, used to express *liveness properties* (i.e., something eventually happens). **ν** (nu) in the standard modal μ-calculus corresponds to **max** in Mads Dam's notation — both denote the greatest fixed point, used to express *safety properties* (i.e., something always holds). We can use both of notations here. The results are shown in the figure bellow.
...
*Figure: Liveness, Schedulability, Dispatch Exclusion*

**Equivalence checking**  (c.f. Figure below)

We perform equivalence checking between the original AADL model given in our paper and its refined AADL model produced by the RAMSES tool. It is supposed that the RAMSES generated model should have the same observational behavior with its original one (it preserves the intended behaviour of the system). This is confimed by checking their corresponding pi-calculus code in MWB. We present bellow the refined AADL Model derived by Ramses, its pi-calculus code generated by our AADLtoPiCalculus tool, and the results of verification in MWB. The result is shown in the figure below.
